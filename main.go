...
func main() {
	...
	for {
		...
		log.Println("Of a total of " + strconv.Itoa(len(slots)) + " slots " + strconv.Itoa(len(valids)) + " are valid slots for booking")
...
func validSlots(slots []DrivingSlot) []DrivingSlot {
	...
	errCheck(err, "Failed to understand the number of days to look ahead days - DAYSTOLOOKAHEAD - from the enviroment")
